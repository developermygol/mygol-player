...
class MatchListItem extends Component {
    ...
    getMatchScore = (match) => {
        ...
        return <Text style={[style.Score]}>{(match.status > 1 && match.status < 10) ? match.homeScore + ' · ' + match.visitorScore : '---'}</Text>
    }
            ...
            <View style={style.View}>
                <TouchableOpacity onPress={this.handlePress} style={null}>
                    ...
                    <View style={style.Row}>
                        ...
                        {this.getTeamNameContent(match.idHomeTeam, homeTeam, match.homeTeamDescription, [style.Name, style.Left])}
                        {this.getMatchScore(match)}
                        {this.getTeamNameContent(match.idVisitorTeam, visitorTeam, match.visitorTeamDescription, [style.Name, style.Right])}

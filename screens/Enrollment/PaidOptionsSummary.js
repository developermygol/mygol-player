        ...
        return (
            <View style={style.Option}>
                ...
                <Text style={style.OptionTitle}>
                    {option.title}: <Text style={style.Price}>{option.price.toFixed(2)} {Localize('CurrencySymbol')}</Text>
                </Text>
            </View>
        )
    }
        ...
        return (
            <View style={GlobalStyles.MainView}>
                ...
                <View style={style.OptionsWrapper}>
                    {selOpts.map(step => <PaidOptionSummaryItem key={step.id} step={step} />)}
                    { fees ? <PaidOptionSummaryItem step={{ title: Localize('Fees.Title'), selectedOption: {title: Localize('Fees.Description'), price: fees } }} /> : null }
                </View>
                
                <View style={style.Horizontal}>
                    <Text style={style.TotalText}>{Localize('Total')}</Text> 
                    <Text style={style.Total}>{(optionsTotal + fees).toFixed(2)} {Localize('CurrencySymbol')}</Text>
                </View>

...
class ScorersRanking extends Component {
    ...
    goToPlayer = (row) => {
        const p = this.props;
        p.navigation.push('PlayerDetails', { idUser: row.idUser, idTeam: row.idTeam, idTournament: p.store.tournaments.current.id });
    }
    ...
    getRank = (row, col, i) => {
        return <Text style={style.Rank} key={col.id} style={{ flex: 1, textAlign: 'center' }}>{i + 1}</Text>;
    }
    ...
    getTeam = (row, col, normalTeams) => {
        if (!normalTeams) return <Text key={col.id} style={{ flex: 4 }}>{Localize('Team.NoTournamentContext')}</Text>

        const team = normalTeams[row.idTeam];
        if (!team) return <Text key={col.id} style={{ flex: 4 }}>{Localize('Team.NoTeamInTournament')}</Text>
                ...
                <ScrollView>
                    <DataTable
                        columns={[
                            ...
                            { id: 2, headerStyle: style.Header1, title: Localize('Points.Short'), style: style.Points, fieldName: 'points' },
                            { id: 3, headerStyle: { flex: 5 }, title: Localize('Player'), renderHandler: this.getPlayerName },
                            { id: 4, headerStyle: { flex: 4 }, title: Localize('Team'), renderHandler: (row, col) => this.getTeam(row, col, normalTeams) }
                        ]}
                        data={p.data}
                        stlye={style.Table}

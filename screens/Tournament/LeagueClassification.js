...
import { requestAsync, getClipEllipsis } from '../../components/Utils';
...
export const GroupClassification = withNavigation(class GroupClass extends Component {
    ...
    renderRank = (row, col, index) => {
        return <Text key={col.id} numberOfLines={1} ellipsizeMode={getClipEllipsis()} style={style.valueImportant}>{index + 1}</Text>
    }
    ...
    render() {
        ...
        return (
            <DataTable
                columns={[
                    { id: 1, fieldName: 'rank', title: '#', headerStyle: style.headerDefault, renderHandler: this.renderRank },
                    { id: 2, fieldName: 'name', title: group.name, headerStyle: style.headerTeamName, renderHandler: this.renderTeamName },
                    { id: 10, fieldName: 'tournamentPoints', title: Localize('TournamentPoints.Short'), headerStyle: style.headerDefault, style: style.valueDefault },
                    { id: 3, fieldName: 'gamesPlayed', title: Localize('GamesPlayed.Short'), headerStyle: style.headerDefault, style: style.valueDefault },
                    { id: 4, fieldName: 'gamesWon', title: Localize('GamesWon.Short'), headerStyle: [style.headerDefault, style.gamesWon], style: style.valueDefault },
                    { id: 5, fieldName: 'gamesDraw', title: Localize('GamesDraw.Short'), headerStyle: [style.headerDefault, style.gamesDraw], style: style.valueDefault },
                    { id: 6, fieldName: 'gamesLost', title: Localize('GamesLost.Short'), headerStyle: [style.headerDefault, style.gamesLost], style: style.valueDefault },
                    { id: 7, fieldName: 'points', title: Localize('Points.Short'), headerStyle: style.headerDefault, style: style.valueDefault },
                    { id: 8, fieldName: 'pointsAgainst', title: Localize('PointsAgainst.Short'), headerStyle: style.headerDefault, style: style.valueDefault },
                    { id: 9, fieldName: 'pointDiff', title: Localize('PointDiff.Short'), headerStyle: style.headerDefault, style: style.valueDefault },
                ]} 
                dataKeyField='name'
...
class LeagueClassification extends Component {
    ...
    render() {
        ...
        if (!leagueClassification || leagueClassification.length === 0) leagueClassification = this.adaptTeamListToClassification(stage);
        ...
        if (groups.length === 1) return (
            <ScrollView>
                <GroupClassification group={groups[0]} normalTeams={p.store.teams.normal} />
            </ScrollView>
        );

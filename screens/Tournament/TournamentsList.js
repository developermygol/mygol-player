        ...
        return (
            <TouchableOpacity onPress={() => onPressItem(item.id)}>
                <View style={style.Item}>
                    <Image source={{uri: getUploadsIcon(item.logoImgUrl, item.id, 'tournament')}} style={style.ItemIcon} />
        ...
        return (
            <Spinner data={data}>
                <View style={GlobalStyles.MainView}>
                    ...
                    <FlatList
                        data={data}
                        renderItem={({item}) => <TournamentListItem item={item} onPressItem={() => this.handlePress(item.id, item.name)} />}

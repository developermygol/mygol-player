...
class SanctionDetails extends Component {
  ...
  render() {
    if (this.noIdProp)
      return (
        <View style={{ flex: 1 }}>
          <InfoBox lMsg="Sanction.NoSanction" />
        </View>
      );
    ...
    const sanctionedMatches = sanction ? sanction.sanctionMatches : null;
        ...
        <View style={style.Details}>
          ...
          <View style={style.Horizontal}>
            ...
            <OutputField
              style={style.Field}
              title={'Player'}
              value={sanction.player && sanction.player.name + ' ' + sanction.player.surname}
            />
          ...
          {sanction.allegations &&
            sanction.allegations.map(a => <SanctionAllegation allegation={a} key={a.id} />)}

          {sanctionedMatches && sanctionedMatches.length > 0 && (
            <>
              <SectionHead title={Localize('Sanction.SanctionedMatches')} />
              <MatchList days={sanctionedMatches} showDaySeparator={false} isFlatMatchList={true} />
            </>
          )}
        </View>
      </ScrollView>
